...
export class YandexTrackerMcpServer extends YandexMcpServer {
  private _api: YandexTrackerReadAPI;
  /**
   * колим контруктор суперкласса
   */
  constructor(name: string, version: string) {
    super(name, version);
    this._api = YandexTrackerReadAPI.getInstance();
    this.addResources();
    this.addPrompts();
    this.addTools();
  }
  ...
  private async getIssueTransitionsToolCallback(
    ...
    try {
      const response: TransitionType[] =
        await this._api.getIssueTransitions(
          args.issueKey
        );
  ...
  private async getIssueChangeLogToolCallback(
    ...
    try {
      const response: ChangelogItemType[] =
        await this._api.getIssueChangeLog(
          args.issueKey,
          args.perPage,
          args.page
        );
  ...
  private async getIssueCheckListToolCallback(
    ...
    try {
      const response: CheckListType[] =
        await this._api.getIssueCheckList(
          args.issueKey,
          args.perPage,
          args.page
        );
  ...
  private async getIssueCommentsToolCallback(
    ...
    try {
      const issueComments: CommentType[] =
        await this._api.getIssueComments(
          args.issueKey,
          args.perPage,
          args.page
        );
  ...
  private async getSprintToolCallback(
    ...
    try {
      const sprint: SprintType = await this._api.getSprint(
        args.sprintId
      );
  ...
  private async getBoardSprintsToolCallback(
    ...
    try {
      const sprints: SprintType[] =
        await this._api.getBoardSprints(args.boardId);
  ...
  private async getQueuesToolCallback(
    ...
    try {
      const queues: Queue[] = await this._api.getQueues({
        expand: args.expand,
      });
  ...
  private async getMySelfToolCallback(
    ...
    try {
      const response: User = await this._api.getMyself();
  ...
  private async getIssueToolCallback(
    ...
    try {
      ...
      const issue: Issue = await this._api.getIssue(
        args.issueKey
      );
  ...
  private async getUserToolCallback(
    ...
    try {
      const user: User = await this._api.getUser(args.key);
  ...
  private async searchIssueByFilterToolCallback(
    ...
    try {
      const issueArray: Issue[] =
        await this._api.searchIssueByFilter(
          args.filter,
          args?.order,
          args.perPage,
          args.page
  ...
  private async searchIssueByQueryToolCallback(
    ...
    try {
      const issueArray: Issue[] =
        await this._api.searchIssueByQuery(
          args.query,
          args.isSimple,
          args.perPage,
          args.page
